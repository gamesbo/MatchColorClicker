...
Shader "Hidden/EditorPaint/Distort" 
{
	Properties
	{
		distortTex("Distortion Texture", 2D) = "white" {}
		distortIntensity("Intensity", Range(-1.0, 1.0)) = 0.05
	...
	SubShader {
		...
		Pass {	
			...
			{
				float4 uvDistortion = tex2D(distortTex, i.texcoord);
				uvDistortion = (uvDistortion - 0.5f) * 2;
				uvDistortion *= distortIntensity;

				float4 color = tex2D(_MainTex, i.texcoord + uvDistortion);
