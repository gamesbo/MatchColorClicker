...
Shader "Hidden/EditorPaint/Blend/Normal" 
{
   Properties
   {
   }
   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         float4 frag( v2f i ) : SV_Target
         {
			...
			output = source1;
			float4 output2 = source2 + (source1 * (1.0f - layerBlendAlpha));  // Premultiplied alpha math
			output = lerp(output, output2, _LayerOpacity);
