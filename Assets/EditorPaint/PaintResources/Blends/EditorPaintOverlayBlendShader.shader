...
Shader "Hidden/EditorPaint/Blend/Overlay" 
{
   Properties
   {
   }
   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         float4 frag( v2f i ) : SV_Target
         {
			...
			source2 = source2 + (source1 * (1.0f - layerBlendAlpha));

			//Overlay math
			float4 overlayResult = ( source1 < 0.5f ? ( 2.0f * source1 * source2) : ( 1.0f - 2.0f * ( 1.0f - source1 ) * ( 1.0f - source2) ) );

			output = lerp(source1, overlayResult, layerBlendAlpha * _LayerOpacity);
