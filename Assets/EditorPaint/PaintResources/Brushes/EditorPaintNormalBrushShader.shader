...
Shader "Hidden/EditorPaint/Brush/Normal" 
{
	Properties
	{
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				output = backTex;
				float4 output2 = ((1.0f - brushTex) * _Color) + (backTex * brushTex);  // Premultiplied alpha math
				output = lerp(output, output2, _Color.a);
				output.a = 1;
		...
		float4 frag(v2f i) : SV_Target
		{
			...
			output = backTex;
			float output2 = (1.0f - brushTex) + (backTex * (brushTex));  // Premultiplied alpha math
			output = lerp(output, output2, _Color.a);
			output.a = 1;
